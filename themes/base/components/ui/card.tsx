...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & { variant?: 'default'|'glass'|'gradient' }>(({ className, variant='default', ...props }, ref) => { const variants = { default:'bg-card text-card-foreground shadow-[var(--shadow-sm)] border', glass:'glass-card text-card-foreground', gradient:'gradient-secondary text-card-foreground shadow-[var(--shadow-md)] border-0' }; return <div ref={ref} className={cn('rounded-xl transition-all duration-300 hover:shadow-[var(--shadow-lg)]', variants[variant], className)} {...props} /> })
Card.displayName = 'Card'
const CardHeader = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => (<div ref={ref} className={cn('flex flex-col space-y-1.5 p-6 pb-4', className)} {...props} />))
